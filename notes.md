...
- [x] Figma always defaults to cloud.
  - This changes the workflow for local use to "save changes by export as .fig"
  - **Resolution: only problem during branch development**
- [x] Some symbols are exporting as 17x16
- [ ] Can't use 'clip content' opition on Frame/Components with current build
  - It inserts an additional sibling node
- [x] `<title>` attribute is not present (similar to new Illustrator output)
  - **Resolution: improved `Symbol` component and `interfaces` helpers**
    - [Symbol README](https://github.com/planningcenter/design/tree/master/planningcenter/symbol)
    - [Interfaces helper](https://github.com/ministrycentered/interfaces/blob/master/app/helpers/interfaces/application_helper.rb#L19-L64)
- [ ] Lack of precision control means exports are up to 4x larger

## Working workflow

(**only during branch development**)
...
- [x] If used, document usage of Figma [Distributed](https://www.figma.com/community/plugin/761098431161143653) plugin
  ...
  - **Resolution: use nested frames instead of plugin (which everyone would have to keep installed)**
- [x] Conduct a11y research on best practices for `<use>` tag SVGs for our listed browser suppert
...
- [x] Add visual regression testing via `Storybook` and `Chromatic`, like `planningcenter/design`
  - **Resolution:**
    - https://www.chromatic.com/builds?appId=5f9ca8cd9bb944002227f01e
- [x] Move remaining global sets into Figma: `product` and `brand`
- [x] Work as Coalitions to move remaining sets into local figma file
